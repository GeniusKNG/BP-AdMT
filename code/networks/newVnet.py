...
class Decoder(nn.Module):
    def __init__(self, n_channels=3, n_classes=2, n_filters=16, normalization='none', has_dropout=False, has_residual=False, up_type=0):
        ...
        # deep supervision
        self.out_conv_2 = nn.Conv3d(n_filters * 2, n_classes, 1, padding=0)
        self.out_conv_3 = nn.Conv3d(n_filters * 4, n_classes, 1, padding=0)
        self.out_conv_4 = nn.Conv3d(n_filters * 8, n_classes, 1, padding=0)
    ...
    def forward(self, features):
        ...
        x5_up = x5_up + x4
        outs.append(self.out_conv_4(x5_up))

        x6 = self.block_six(x5_up)
        x6_up = self.block_six_up(x6)
        x6_up = x6_up + x3
        outs.append(self.out_conv_3(x6_up))
        

        x7 = self.block_seven(x6_up)
        x7_up = self.block_seven_up(x7)
        x7_up = x7_up + x2
        outs.append(self.out_conv_2(x7_up))

        x8 = self.block_eight(x7_up)
        x8_up = self.block_eight_up(x8)
        x8_up = x8_up + x1

        valid_feat.append(x8_up)
        valid_feat.append(x7_up)
        valid_feat.append(x6_up)
        valid_feat.append(x5_up)
...
class VNet(nn.Module):
    def __init__(self, n_channels=3, n_classes=2, n_filters=16, normalization='none', has_dropout=False, has_residual=False, up_type=0):
        ...
        self.decoder = Decoder(n_channels, n_classes, n_filters, normalization, has_dropout, has_residual, up_type)

    def forward(self, input, deepsup=False):
        features = self.encoder(input)
        outs, valid_feat = self.decoder(features)
        return (outs, valid_feat) if deepsup else (outs[-1], valid_feat)




if __name__ == '__main__':
        x = torch.randn(1,1,112,112,80).cuda()
        model = VNet(n_channels=1,n_classes=2).cuda()
        outs, validf = model(x, deepsup=True)
        for out in zip(outs,validf):
