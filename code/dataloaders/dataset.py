...
class LAHeart_no_read(Dataset):
    ...
    def __init__(self, base_dir=None, split='train', num=None, transform=None,with_idx=False):
        ...
        for i in range(len(self.image_list)):
            ...
            self.image_list_data.append(h5py.File(self._base_dir + "/" + self.image_list[i] + "/mri_norm2.h5", 'r'))
...
class BraTS2019_no_read(Dataset):
    def __init__(self, base_dir=None, split='train', num=None, transform=None,with_idx=False):
        ...
        for i in range(len(self.image_list)):
            self.image_list_data.append(
                h5py.File(self._base_dir + "/BraTSh5/{}.h5".format(self.image_list[i]), 'r'))
...
#       primary_iter = iterate_once(self.primary_indices)
        primary_iter = iterate_once_fixed_batch(self.primary_indices,self.primary_batch_size)
...
#       return len(self.primary_indices) // self.primary_batch_size
        return math.ceil(len(self.primary_indices) / self.primary_batch_size)

def iterate_once(iterable):
    return np.random.permutation(iterable)

def iterate_once_fixed_batch(iterable,label_bs):
    indices = np.random.permutation(iterable)
    n = len(indices)
    if n % label_bs != 0:
        indices = np.append(indices, np.random.choice(indices,size=label_bs-(n % label_bs)))
