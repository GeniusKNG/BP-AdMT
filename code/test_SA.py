...
def test_all_case(model, num_classes, patch_size=(112, 112, 80), stride_xy=18, stride_z=4, dataset_name="LA", save_result=False, test_save_path=None):
    if dataset_name == "LA":
    ...
    for image_path in loader:
        if dataset_name == "LA":
            image_name = image_path.split('/')[-2] + '.h5'
        elif dataset_name == "Pancreas_CT":
        ...
        if save_result:
            save_path = os.path.join(test_save_path, image_name)
...
parser.add_argument('--exp', type=str,  default='SA-AMT', help='exp_name')
parser.add_argument('--labelnum', type=int, default=8, help='labeled data')
parser.add_argument('--model', type=str,  default='vnet', help='model_name')
parser.add_argument('--sup_only', type=int, default=0, help='whether supervised training only')
parser.add_argument('--ad_ema', type=int, default=1, help='whether to use adaptive ema')
parser.add_argument('--ad_mix', type=int, default=1, help='whether to use adaptive mix')
...
#     FLAGS.model = 'vnets'
    model_path = FLAGS.root_path + "model/{}_{}_{}_labeled_suponly{}/{}/ad_ema{}-ad_mix{}-almax{}-almin{}-savefreq{}-topk{}-per_num{}-weight_psemix{}-adamw-{}-{}/{}_{}_best_model.pth".format(FLAGS.dataset_name, FLAGS.exp, FLAGS.labelnum, FLAGS.sup_only, FLAGS.model, FLAGS.ad_ema, FLAGS.ad_mix, FLAGS.alpha_max, FLAGS.alpha_min, FLAGS.savefreq, FLAGS.topk, FLAGS.per_num, FLAGS.weight_psemix, FLAGS.adamw_lr, FLAGS.decay, FLAGS.exp, FLAGS.iter)
else:
    model_path = FLAGS.root_path + "model/{}_{}_{}_labeled_suponly{}/{}/ad_ema{}-ad_mix{}-almax{}-almin{}-savefreq{}-topk{}-per_num{}-weight_psemix{}-adamw-{}-{}/{}_best_model.pth".format(FLAGS.dataset_name, FLAGS.exp, FLAGS.labelnum, FLAGS.sup_only, FLAGS.model, FLAGS.ad_ema, FLAGS.ad_mix, FLAGS.alpha_max, FLAGS.alpha_min, FLAGS.savefreq, FLAGS.topk, FLAGS.per_num, FLAGS.weight_psemix, FLAGS.adamw_lr, FLAGS.decay, FLAGS.exp)
...
if __name__ == '__main__':
        ...
        model.eval()
        test_all_case(model=model, num_classes=num_classes, patch_size=patch_size, stride_xy=stride_xy, stride_z=stride_z, dataset_name=FLAGS.dataset_name, save_result=True, test_save_path=test_save_path)
