...
parser.add_argument('--ad_ema', type=int, default=0, help='whether to use adaptive ema')
parser.add_argument('--ad_mix', type=int, default=0, help='whether to use adaptive mix')
parser.add_argument('--alpha', type=float, default=0.99, help='default alpha value')
...
snapshot_path = args.root_path + "model/{}_{}_{}_labeled_suponly{}/{}/ad_ema{}-ad_mix{}-almax{}-almin{}-savefreq{}-topk{}-per_num{}-weight_psemix{}-adamw-{}-{}".format(args.dataset_name, args.exp, args.labelnum, args.sup_only,
                                                                    args.model,args.ad_ema, args.ad_mix, args.alpha_max, args.alpha_min, args.savefreq, args.topk, args.per_num, args.weight_psemix, args.adamw_lr, args.decay)
...
#     logging.info("%s alpha=%s", args, alpha)
    ...
    adamix = AdaMix3D_boud(total_steps=max_iterations // iter_per_epoch, num_classes=num_classes, image_size=patch_size, patch_size=args.per_num, topk=args.topk)
