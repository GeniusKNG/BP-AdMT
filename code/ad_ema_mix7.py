...
#     logging.info("%s alpha=%s", args, alpha)
    ...
    for epoch_num in iterator:
        ...
        #     U_mix_label = Lmix.cuda().float()
            ...
            U_data_m, U_mix_label, pred_l_conf, adloss, tok_list = adamix(oimage=U_data, aimage=X_data, olabel=pseudo_label.long(), alabel=X_label.long(), oconf=conf_u, aconf=conf_l, prediction=output_all[args.labeled_bs:args.labeled_bs*2], cur_step= epoch_num)
        ...
        #     writer.add_scalar('jsd', jsd, iter_num)
            ...
            if iter_num >= 6000 and iter_num % args.savefreq == 0:                
                now = datetime.now()
