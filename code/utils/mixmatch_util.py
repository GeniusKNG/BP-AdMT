...
def mix_match_cross_select_pseudolabel2(X, U, eval_net, K, alpha, mixup_mode, aug_factor):
    # X [(data[1,D,H,W],label[D,H,W]) * B] , U [B,C,D,H,W]

    X_b = len(X)
    U_b = len(U)
    
    X_cap = [(augmentation_torch(x[0], aug_factor), x[1]) for x in X]
    D_x = [x[0] for x in X_cap]
    X_in = torch.stack(D_x,dim=0)
    U_cap = U.repeat(K, 1, 1, 1, 1)  # [K*b, 1, D, H, W]
    U_cap += torch.clamp(torch.randn_like(U_cap) * 0.1, -0.2, 0.2)  # augmented.
    ...
    with torch.no_grad():
        ...
        t_u = eval_net(U_cap)  # (K*b, n_class, 112, 112, 80)
        t_l = eval_net(X_in)  # (K*b, n_class, 112, 112, 80)
        Y_u = F.softmax(t_u, dim=1)
        Y_l = F.softmax(t_l, dim=1)

    
    pseudo_prob = Y_u
    dl_prob = Y_l
    
    guessed = torch.zeros(U.shape).repeat(1, K, 1, 1, 1)  # [U_b, K, D, H, W]
    # if GPU:
    guessed = guessed.cuda()
    if K !=1:
        for i in range(K):
            guessed += pseudo_prob[i * U_b:(i + 1) * U_b]
        guessed /= K  #
        guessed = guessed.repeat(K, 1, 1, 1, 1)
        guessed = torch.argmax(guessed, dim=1)
    else:

        guessed= torch.argmax(pseudo_prob, dim=1)
    dl_label = torch.argmax(dl_prob, dim=1)
    D_l = [d for d in dl_label]
    
    X_mix = list(zip(D_x, D_l))
    
    pseudo_label = guessed
    # print('pseulabel',pseudo_label.shape)
    # U_cap = U_cap.detach().cpu().numpy()
    ...
    if u_mixup_mode == 'x':  #
        idxs = np.random.permutation(range(U_b * K)) % X_b
        U_prime = [mix_up(U_cap[i], X_mix[idxs[i]], alpha) for i in range(U_b * K)]
        # print('uprime',len(U_prime))
        # print('uprime0',U_prime[0][0].shape,U_prime[0][1].shape)
    elif u_mixup_mode == 'u':  #
        idxs = np.random.permutation(range(U_b * K))
        U_prime = [mix_up(U_cap[i], U_cap[idxs[i]], alpha) for i in range(U_b * K)]  # 有問題???
    elif u_mixup_mode == '_':
        ...
        raise ValueError('wrong mixup_mode')
    Umix = torch.stack([u[0] for u in U_prime], dim=0)
    Lmix = torch.argmax(torch.stack([u[1] for u in U_prime], dim=0),dim=1)
    
    return X_prime, Umix, Lmix, pseudo_label, dl_label, X_cap, t_u, t_l
...
def to_one_hot(mask, num_classes):
    ...
    return: [B, C, H, W]
    """
    return F.one_hot(mask.long(), num_classes).permute(3, 0, 1, 2).float()
...
def mix_up(s1, s2, alpha):
    ...
    x1, p1 = s1
    x2, p2 = s2
    p1 = to_one_hot(p1, num_classes=2)
    p2 = to_one_hot(p2, num_classes=2)
